...
logger.log_info("开始 Stage2: FiLM调制训练")
...
# 加载Stage1的模型权重
try:
    model.load_state_dict(torch.load('checkpoints/stage1_epoch30.pth'))
    logger.log_info("成功加载Stage1模型权重")
except:
    logger.log_info("未找到Stage1模型权重，使用随机初始化")
...
logger.log_info("Stage2: FiLM调制训练完成")